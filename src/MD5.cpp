...
void MD5::update(const uint8_t input[], uint32_t length) {
    ...
    uint32_t index = m_count / 8 % kBlockSize;
    ...
    if (length >= firstPart) {
        ...
        std::memcpy(&m_buffer[index], input, firstPart);
        transform(m_buffer);
...
void MD5::finalize() {
    ...
    if (!m_finalized) {
        ...
        encode(reinterpret_cast<uint8_t*>(&m_digest), m_state, 16);
...
// return digest
uint128_t MD5::getDigest() { return m_digest; }
